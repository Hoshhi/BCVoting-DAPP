 ...
 */
contract BCVoting {
    uint256 constant MIN_DEPOSIT_AUTHORITY = 10000;
    uint256 constant DEPOSIT_VOTER = 1000;
    uint256 constant MOD_MAX = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
    ...
    uint256 public sizeHash; //size derived from public modulus (in Bytes)
    int256[] public tally; // results of voting; associated with candidates

    // Voter[] public voters; // Data of eligible voters.
    mapping(address => uint256) public deposits; // Deposits serve for handling cases such as misbehaving authority or non-voting participants
    ...
    ]; // common generator for voters - base points
    uint256 public NN =
        0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141; // modulus for private keys (i.e., order of G)
    uint256 public PP =
        0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F; // modulus for public keys (i.e., field size)
    uint256 public A = 0;
    uint256 public B = 7;
    uint256 public Lambda =
        0x5363ad4cc05c30e0a5261c028812645a122e22ea20816678df02967c1b23bd72; // a root of the characteristic polynomial of the endomorphism of the curve
    uint256 public Beta =
        0x7ae96a2b657c07106e64479eac3434e99cf0497512f58995c1396c28719501ee; // constant on the curve (endomorphism)

    // Other Crypto stuff
    uint256[2][] public ephemeralPKs; // items are G^x; array is indexed by idx of voter present in idxVoters mapping.
    uint256[2][] public MpcPKs; // items are G^y (or yG for EC)
    // uint[] public MpcPKsTmp; // only for debug purposes; delete later
    // uint[] public blindedVotes; // it contains pairs
    mapping(uint256 => uint256[2]) public blindedVotes;
    uint256 blindedVotesCnt = 0; // counter of blinded votes submitted
    uint256[2][] public candidatesG; // generators for candidates (denoted as f)
    uint256[] public notVotedIdxs; // idx of voters who did not voted despite submitting ephemeral PK

    // Timeouts
    uint256 public timeout_SignUp;
    uint256 public timeout_PreVoting;
    uint256 public timeout_Voting;
    uint256 public deltaT; // step for timeouts
    ...
    mapping(address => uint256) public idxVoters; // mapping of addresses of voters to index in ephemeralPKs array (for fast access)
    // mapping (uint => bool) public votersVoted; // indication which voters already voted
    mapping(uint256 => bool) public submitted_BlindedKeys;
    uint256 submittedVotersFT = 0;

    mapping(uint256 => bool) usedGens;
    enum StageEnum {SETUP, SIGNUP, PRE_VOTING, VOTING, FT_RESOLUTION, TALLY}

    // State preservation for batching of MPC key computation
    uint256 public MPC_start = 0; // idx of the voter that is at the begining of batching sliding window
    uint256 public MPC_batch; // the size of the batch
    uint256[3] public MPC_act_left = [G[0], G[1], 1];
    uint256[3][] public MPC_right_markers;

    // EVENTS
    event SingUpFinished();
    // event VotingFinished(uint[] submittedVotePairs);
    event TallyCompleted(int256[] tally);
    event ArraysCompared(bytes a1, bytes a2);
    event MPCKeysComputedEvent(uint256[] mpcKeys1D);
    event MPCKeysComputedEvent();
    event HashComputedEvent(uint256 h);
    event FirstConditionEvent(uint256[3] left, uint256[3] right);
    event VotingExpiredWithMissingVotes(uint256[] notVotedIdxs);
    event RepairedBVoteEvent(uint256[2] blindedVote);
    ...
    modifier voterVoted(address voter, uint256 myIdx) {
        require(
            idxVoters[voter] == myIdx,
            "Submitted index does not correspond to the address of voter."
        );
        require(blindedVotes[myIdx][0] != 0, "Voter did not vote.");
        _;
    }
    modifier isVoterMissing(uint256 faultyIdx) {
        require(
            blindedVotes[faultyIdx][0] == 0,
            "Voter marked as faulty is not faulty."
        );
        _;
    }
    modifier notSubmittedBlindingKeyYet(uint256 myIdx) {
        require(
            !submitted_BlindedKeys[myIdx],
            "Voter already submitted blinding key."
        );
        _;
    }
    ...
    modifier checkDepositSentByAuthority() {
        require(
            msg.value >= MIN_DEPOSIT_AUTHORITY,
            "Not enough deposit sent by authority."
        );
        _;
    }
    modifier checkDepositSentByVoter() {
        require(
            msg.value == DEPOSIT_VOTER,
            "Incorrect deposit sent by a voter."
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            _cand_generators.length == _candidates.length * 2,
            "Candidates and generators must have the same length"
        );
        deltaT = _deltaT;
        ...
        for (uint256 i = 0; i < _candidates.length; i++) {
            ...
            candidatesG.push(
                [_cand_generators[i * 2], _cand_generators[i * 2 + 1]]
            ); // extract consecutive pairs of x_i,y_i values from 1D array
            require(
                EC.isOnCurve(
                    candidatesG[candidatesG.length - 1][0],
                    candidatesG[candidatesG.length - 1][1],
                    A,
                    B,
                    PP
                ),
                "Candidate generator must be on the curve."
            );
     ...
     */
    function enrollVoters(address[] memory _voters)
        public
        payable
        verifySigAuthority()
     ...
     */
    function submitEphemeralPK(uint256[2] memory ephemeralKey)
        public
        payable
    ...
    {
        require(
            EC.isOnCurve(ephemeralKey[0], ephemeralKey[1], A, B, PP),
            "Ephemeral PK of the voter must be on the curve."
        );
     ...
     */
    function changeStageToPreVoting() public inStage(StageEnum.SIGNUP) {
        require(
            expiredTimeout(timeout_SignUp),
            "Sing-up timeout has not expired yet."
        );
        stage = StageEnum.PRE_VOTING;
        emit SingUpFinished();
        timeout_PreVoting = block.timestamp + deltaT;
    ...
    {
        require(
            0 == MPC_start && 0 == MPC_right_markers.length,
            "Righ markers has already been built."
        );
        require(
            ephemeralPKs.length >= MPC_batch,
            "MPC batch size is higher than the number of voters (should be equal or lower). "
        );
        ...
        uint256[3] memory right_tmp = [G[0], G[1], 1];

        if (0 != ephemeralPKs.length % MPC_batch) {
            // deal with a special case where ephemeralPKs.length is not divisible by the batch size
        ...
        for (uint256 j = 1; j < ephemeralPKs.length; j++) {
            if ((ephemeralPKs.length % MPC_batch) == (j - 1) % MPC_batch) {
            ...
            (right_tmp[0], right_tmp[1], right_tmp[2]) = EC.OVN_addMixed(
                right_tmp,
                ephemeralPKs[ephemeralPKs.length - j],
                A,
                PP
            );
        }
     ...
     */
    function computeMPCKeys(
        uint256[] memory inv_mod_mpc2,
        uint256[] memory inv_mod_mpc1
    ) public verifySigAuthority() inStage(StageEnum.PRE_VOTING) {
        require(
            !expiredTimeout(timeout_PreVoting),
            "PreVoting timeout expired"
        );
        require(
            0 != MPC_right_markers.length,
            "Righ markers has not been built yet."
        );
        require(
            MPC_batch == inv_mod_mpc1.length ||
                ephemeralPKs.length % MPC_batch == inv_mod_mpc1.length,
            "Length of mod inverse cache array 1 is wrong."
        );
        require(
            MPC_batch == inv_mod_mpc2.length ||
                ephemeralPKs.length % MPC_batch == inv_mod_mpc2.length,
            "Length of mod inverse cache array 2 is wrong."
        );

        uint256[3] memory act_left_m = MPC_act_left;
        uint256[3][] memory right_table = new uint256[3][](MPC_batch);

        // build the right table using markers created before
        uint256 skipOffset = 0;
        if (MPC_start + MPC_batch > ephemeralPKs.length) {
        ...
        right_table[MPC_batch - skipOffset - 1] = MPC_right_markers[
            MPC_right_markers.length - MPC_start / MPC_batch - 1
        ];

        for (uint256 j = 1 + skipOffset; j < MPC_batch; j++) {
            uint256 idx = MPC_batch - j;
        ...
        uint256 end_bound = MPC_start + MPC_batch;
        ...
        for (uint256 i = MPC_start; i < end_bound; i++) {
            if (0 != i) {
                // accumulate sum of x_j * G, starting by the first voter
                (act_left_m[0], act_left_m[1], act_left_m[2]) = EC.OVN_addMixed(
                    act_left_m,
                    ephemeralPKs[i - 1],
                    A,
                    PP
            ...
            (res[0], res[1], res[2]) = EC.toAffine3_optim(
                res[0],
                res[1],
                res[2],
                inv_mod_mpc2[i - MPC_start],
                PP
            );
     ...
     */
    function submitVote(
        uint256[] memory proof_A,
        uint256[] memory proof_B,
        int256[] memory proof_r,
        int256[] memory proof_d,
        uint256[2] memory vote,
        uint256[] memory mod_invs
    ) public voterSubmittedEphPK(msg.sender) inStage(StageEnum.VOTING) {
        ...
        uint256 dxor = 0; // xor of d_l paramteres
        for (uint256 i = 0; i < candidates.length; i++) {
            uint256 proof_d_full;
            // compute d = d1+d2*LAMBDA (mod n) to ensure consistency as part of later hash verification
            if (proof_d[2 * i + 1] < 0) {
                proof_d_full =
                    NN -
                    mulmod(uint256(-proof_d[2 * i + 1]), Lambda, NN);
            } else {
                proof_d_full = mulmod(uint256(proof_d[2 * i + 1]), Lambda, NN);
            }

            if (proof_d[2 * i] < 0) {
                proof_d_full = addmod(
                    NN + uint256(-proof_d[2 * i]),
                    proof_d_full,
                    NN
                );
            } else {
        ...
        require(
            areArraysEqual(
                truncate(
                    abi.encodePacked(
                        keccak256(abi.encodePacked(proof_A, proof_B))
                    ),
                    sizeHash
                ),
        ...
        uint256 idxVoter = idxVoters[msg.sender];
        uint256[3] memory left;
        uint256[6] memory right; // right[3-5] is another tmp variable (since stack deep error occurs; allowing only 16 local vars and func params)
        ...
        for (uint256 l = 0; l < candidates.length; l++) {
            ...
            (left[0], left[1], left[2]) = FastEcMul.ecSimMul(
                ...
                [
                    ...
                    G[1],
                    1,
                    ephemeralPKs[idxVoter][0],
                    ephemeralPKs[idxVoter][1],
                    1
                ],
                A,
                Beta,
            ...
            (left[0], left[1], left[2]) = EC.toAffine3_optim(
                left[0],
                left[1],
                left[2],
                mod_invs[l * 3],
            ...
            require(
                left[0] == proof_A[2 * l] && left[1] == proof_A[2 * l + 1],
                "Proof verification failed at the 1st condition."
            );
            ...
            (right[0], right[1], right[2]) = EC.toAffine3_optim(
                right[0],
                right[1],
                right[2],
                mod_invs[l * 3 + 2],
            ...
            require(
                ...
                "Proof verification failed at 2nd condition."
            );
        }
     ...
     */
    function changeStageToFT_Resolution() public inStage(StageEnum.VOTING) {
        require(
            expiredTimeout(timeout_Voting),
            "Voting timeout has not expired yet."
        );
        stage = StageEnum.FT_RESOLUTION;

        for (uint256 i = 0; i < ephemeralPKs.length; i++) {
     ...
     */
    function FT_repairBlindedVote(
        uint256[] memory mod_invs,
        int256[] memory proof_r,
        int256[] memory h_decomp,
        uint256[] memory proof_m1,
        uint256[] memory proof_m2,
        uint256[] memory blindedKeys,
        uint256[] memory faultyIdx,
        uint256 myIdx
    )
        public
        ...
        notSubmittedBlindingKeyYet(myIdx)
    {
        ...
        for (uint256 f = 0; f < faultyIdx.length; f++) {
            require(
                blindedVotes[faultyIdx[f]][0] == 0,
                "Voter marked as faulty is not faulty."
            );
            ...
            if (faultyIdx[f] < myIdx) {
                // blinded key has positive sign (it is on the left from my index) => we need to subtract it from blinded vote
            ...
            (blindedVotes[myIdx][0], blindedVotes[myIdx][1]) = EC.ecAdd(
                blindedVotes[myIdx][0],
                blindedVotes[myIdx][1],
                blindedKeys[2 * f],
                blindedKeys[2 * f + 1],
                A,
                PP
            ...
            if (f > 0 && faultyIdx[f - 1] >= faultyIdx[f]) {
                // ascended ordering must be preserved
                revert(
                    "Ascended ordering of faulty indices must be preserved."
                );
            }
        }
        require(
            faultyIdx.length == notVotedIdxs.length,
            "Not all faulty IDXes were processed."
        );
    ...
    function FT_verifyZKP(
        uint256[2] memory mod_invs,
        uint256[2] storage I,
        uint256[2] storage J,
        int256[2] memory proof_r,
        int256[2] memory h_decomp, // decomposed scalars
        uint256[2] memory proof_m1,
        uint256[2] memory proof_m2,
        uint256[2] memory blindedKey
    ) private {
        uint256[3] memory left;
        uint256 h =
            uint256(keccak256(abi.encodePacked(I, J, proof_m1, proof_m2)));
        ...
        require(
            left[0] == proof_m1[0] && left[1] == proof_m1[1],
            "FT Proof verification failed at the 1st condition."
        );
        ...
        require(
            left[0] == proof_m2[0] && left[1] == proof_m2[1],
            "FT Proof verification failed at the 2nd condition."
        );
    }
     ...
     */
    function computeTally(int256[] memory c_decom, uint256[2] memory modinv)
        public
        verifySigAuthority()
        inStage(StageEnum.TALLY)
    {
        require(
            candidates.length * 2 == c_decom.length,
            "C array has incorrect length."
        );

        // Verify that sum of B_i == f_1 * c_1 + f_2 * c_2 * ... * f_k * c_k

        // Compute Sum of B_i
        uint256[3] memory left = [G[0], G[1], 1];
        for (uint256 i = 0; i < ephemeralPKs.length; i++) {
            if (blindedVotes[i][0] == 0) {
                continue;
            }
        ...
        uint256[3] memory sum = [G[0], G[1], 1];
        uint256[3] memory right;
        for (uint256 l = 0; l < candidatesG.length; l += 2) {
            // pairwise iteration
            if (l == candidatesG.length - 1) {
                // in the case of the odd number of candidates, use neutral feature as the second item for multiplication when processing the last candidate
                (right[0], right[1], right[2]) = FastEcMul.ecSimMul(
                ...
                tally[l + 1] = c_decom[2 * l + 2] * c_decom[2 * l + 3]; // (2nd cand in the pair) // this can be switched off as an gas optimization
            }
        ...
        (sum[0], sum[1], sum[2]) = EC.toAffine3_optim(
            sum[0],
            sum[1],
            sum[2],
            modinv[1],
        ...
        require(
            left[0] == sum[0] && left[1] == sum[1],
            "Incorrect tally values were provided."
        );
    ...
    function getEPKs() public view returns (uint256[2][]) {
        return ephemeralPKs;
    }

    function getCntOfEligibleVoters() public view returns (uint256) {
        return eligibleVotersAddrs.length;
    }

    function getCntOfCandidates() public view returns (uint256) {
        return candidates.length;
    }

    function getCntOfEphPKs() public view returns (uint256) {
        return ephemeralPKs.length;
    }

    function getCntOfMarkersMPC() public view returns (uint256) {
        return MPC_right_markers.length;
    }

    function getCntOfMpcPKs() public view returns (uint256) {
        return MpcPKs.length;
    }

    function getCntOfBlinedVotes() public view returns (uint256) {
        return blindedVotesCnt;
    }

    function getBlinedVote(uint256 idx)
        public
        view
    ...
    function expiredTimeout(uint256 t) public view returns (bool) {
     ...
     */
    function truncate(bytes memory hash, uint256 size)
        internal
        pure
    ...
    function truncateLeadingZeros(bytes32 arg, uint256 size)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function areArraysEqual(bytes memory a, bytes memory b)
        internal
        pure
        returns (bool)
    {
    ...
    function modInvCache4MPCBatched(
        uint256 start_idx,
        uint256[3] memory last_left
    )
        public
        view
        inStage(StageEnum.PRE_VOTING)
        ...
        )
    {
        ...
        require(
            start_idx < ephemeralPKs.length,
            "Start idx must be < than the number of participants."
        );

        uint256 arrSize = MPC_batch;
        ...
        // build the right table using markers created before
        uint256 skipOffset = 0;
        if (start_idx + MPC_batch > ephemeralPKs.length) {
            // skipoffset only applies in the last batch of the case where MPC_batch does not divide ephemeralPKs.length
            skipOffset = (MPC_batch - (ephemeralPKs.length % MPC_batch));
        }
        right_table[MPC_batch - skipOffset - 1] = MPC_right_markers[
            MPC_right_markers.length - start_idx / MPC_batch - 1
        ];
        ...
        for (uint256 j = 1 + skipOffset; j < MPC_batch; j++) {
            ...
            (
                ...
                right_table[idx],
                ephemeralPKs[start_idx + idx],
                A,
                PP
            );
        }

        uint256[] memory modInv1 = new uint256[](arrSize);
        uint256[] memory modInv2 = new uint256[](arrSize);
        uint256[3] memory res;
        uint256[3] memory act_left = last_left;

        // handle the end bound for the main cycle
        uint256 end_bound = start_idx + MPC_batch;
        if (end_bound > ephemeralPKs.length) {
            end_bound = ephemeralPKs.length;
        }

        // build cache of modular inverses for the current batch
        for (uint256 i = start_idx; i < end_bound; i++) {
            if (0 != i) {
                // accumulate sum of x_j * G, starting by the first voter
                (act_left[0], act_left[1], act_left[2]) = EC.OVN_addMixed(
                    act_left,
                    ephemeralPKs[i - 1],
                    A,
                    PP
            ...
            modInv1[i % MPC_batch] = EC.invMod(
                right_table[i % MPC_batch][2],
                PP
            ); // store modInv for substraction
    ...
    function modInvCache4SubmitVote(
        ...
        uint256[3][] memory modInv = new uint256[3][](candidates.length);

        uint256 idxVoter = idxVoters[msg.sender];
        uint256[3] memory left;
        uint256[6] memory right; // right[3-5] is another tmp variable (since stack deep occurs; allowing only 16 local vars and func params)
        ...
        uint256[] memory ret1 = new uint256[](candidates.length * 3);
        for (uint256 i = 0; i < candidates.length; i++) {
            ret1[i * 3] = modInv[i][0];
    ...
    function modInvCache4repairVote(
        ...
        uint256[2][] memory modInv = new uint256[2][](faultyIdx.length);
        ...
        uint256[] memory ret1 = new uint256[](faultyIdx.length * 3);
        for (uint256 i = 0; i < faultyIdx.length; i++) {
            ret1[i * 2] = modInv[i][0];
     ...
     */
    function modInvCache4Tally(int256[] memory c_decom)
        public
        view
        returns (uint256[2] memory)
    {
