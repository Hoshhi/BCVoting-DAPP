...
class Utils {
    ...
    sort = function (numArr) {
        return _.sortBy(numArr, function (num) { return num; })
    }
    ...
    BIarrayInvertAndHex = function (arr, NN) {

        assert(Array.isArray(arr));
        let ret = [];
        arr.forEach(e => {
            var inverted = (NN - e) % NN;
            ret.push(W3.utils.numberToHex(inverted.toString(10)));
        });
        return ret;
    }
